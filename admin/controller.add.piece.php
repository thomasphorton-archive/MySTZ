<?php session_start();
	...
	$sql = "INSERT INTO roster SET artist_name=?, facebook_url=?, twitter_url=?, youtube_url=?, soundcloud_url=?, hypem_url=?, back_pic_url=?, front_pic_url=?, bio=?, hidden=?";
	$q = $pdo->prepare($sql);
	$q->execute(array($artist_name,$facebook_URL,$twitter_URL,$youtube_URL,$soundcloud_URL,$hypem_URL,$back_pic_URL,$front_pic_URL,$bio,$hide_flag));
?>
	<div id=wrapper>
		<?=$artist_name;?> has been added to the roster.
