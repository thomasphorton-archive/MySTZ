...
function fn_simple_ultimate(&$request)
{
    ...
    if (Registry::get('runtime.company_id') == 0 || Registry::get('runtime.companies_available_count') == 1) {
        ...
        if (count($companies) == 1) {
            ...
            Registry::set('runtime.forced_company_id', $company_id);
...
function fn_ult_update_lang_values(&$lang_data, &$lang_code, &$error_flag, &$params)
{
    if (Registry::get('runtime.company_id')) {
...
function fn_ult_delete_language_variables(&$names)
{
    if (!empty($names)) {
        if (Registry::get('runtime.company_id')) {
            db_query("DELETE FROM ?:ult_language_values WHERE name IN (?a) AND company_id = ?i AND lang_code = ?s", $names, Registry::get('runtime.company_id'), DESCR_SL);
        } else {
...
{
    $section_condition[] = 's.company_id ' . (Registry::get('runtime.company_id') ? db_quote(' = ?i', Registry::get('runtime.company_id')) : ' IS NULL');
}
...
function fn_init_store_params_by_host(&$request, $area = AREA)
{
    ...
    if (!empty($request['switch_company_id']) && $request['switch_company_id'] != 'all' && !isset($request['skip_config_changing'])) { // theme for company with id = 0 cannot be loaded.
        ...
        return array('BROKEN UNIT TESTS HERE', '', 'No storefronts defined');
    }
 ...
 * @return boolean Always true
 */
function fn_ult_get_customer_css_post(&$files)
{
    if (!defined('COMPANY_ID')) {
        // if company is not defined list of customer styles is empty
        $files = array();
    }
